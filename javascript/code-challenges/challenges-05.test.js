...
const toLastNames = people => {
  const newPeople = people.map( name => {
...
const addValues = (arr) => {
  const totVal = arr.reduce((total, value) => total + value, 0);
  return totVal;
};
...
const addPurchases = (arr) => {
  const cost = arr.reduce( (total, value) => {
...
const countNumberOfElements = (arr) => {
  const numElem = arr.reduce((length) => {
    return length + 1;
  });
  return numElem;
};
...
const returnNames = (arr) => {
  const charArr = arr.reduce((charArr, charObj) => {
    charArr.push(charObj.name);
    return charArr;
  }, []);
  return charArr;
};
...
const reversedString = (str) => {
  ...
  const nextString = firstString.reduce((acc, val) => {
    return val + acc;
  }, '');
  return nextString;
};
